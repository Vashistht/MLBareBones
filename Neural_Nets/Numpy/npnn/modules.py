...
class ELU(Module):
    ...
    def forward(self, x, train=True):
        ...
        for i in range(self.x.shape[0]): # apply elu each element in x
            # if self.x[i]> 0:
            #     x_k[i]= self.x[i]
            # else:
            #     x_k [i]= self.alpha * (np.exp(self.x[i] )- 1)
            x_k[i]= np.where( x[i]>0, x[i], self.alpha * (np.exp(x[i] )- 1))
        return x_k
    ...
    def backward(self, grad):
        ...
        """
        # dLdx = None
        dLdx = np.zeros(grad.shape)
        for i in range(self.x.shape[0]): # apply elu each element in x
            # if self.x[i]> 0:
            #     x_k[i]= self.x[i]
            # else:
            #     x_k[i] = self.alpha * (np.exp(self.x[i] )- 1)
            # only diag elements survive
            # x_k_i = np.diag( np.where( self.x[i]>0, 1, self.alpha * (np .exp(self.x[i] )- 1)) )
            x_k_i = np.diag( np.where( self.x[i]>0, 1, self.alpha * np.exp(self.x[i])) )
            dLdx [i] = grad[i] @ x_k_i
...
class Dense(Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        b = np.zeros(dim_out)
        W = np.random.uniform(-u, u, size = (dim_out,dim_in) )
    ...
    def backward(self, grad):
        ...
        # W.grad = dL/dW_k = ?
        W.grad = (grad.T @self.x)/batch # make sure /batch and mean do the same thing
        # b.grad = dL/db_k = ?
